...
### Planned
- Полноценный live онлайн PvP с pub/sub push-уведомлениями (сейчас beta — соперник видит ход после клика)
- Экспорт PGN
...
Три режима игры, расширенные уровни сложности и стабильное обновление доски.

### Added
- **Hot-seat режим**: «👥 Двое на одном устройстве» — игроки ходят по очереди на одном телефоне, доска переворачивается после каждого хода
- **Онлайн PvP** (beta): «🌐 Онлайн PvP» — создайте игру и поделитесь 6-значным кодом, или присоединитесь по коду. `OnlineService` хранит партии в Redis (TTL 24 ч), синхронизирует ходы и публикует уведомления в Redis pub/sub каналах
- **8 уровней сложности**: Новичок, Лёгкий, Простой, Средний, Продвинутый, Сложный, Эксперт, Гроссмейстер — с разными параметрами Stockfish (skill 0…20, depth 2…24, time 80…5000 ms) и ELO противника от 500 до 2600
- Каркас сервиса `app/game/online.py` для двух-игроковых партий с инвайт-кодами

### Changed
- **Иконки фигур для обоих цветов**: белые `♔♕♖♗♘♙`, чёрные `♚♛♜♝♞♟` (стандартный Unicode-набор)
- **Стабильное обновление доски**: ошибка `message is not modified` (Telegram BadRequest) больше не ломает UX — обернули `edit_text` / `edit_reply_markup` в `_safe_edit_*` хелперы, к каждому ходу добавляется уникальный счётчик в тексте
- Главное меню переработано: 4 кнопки режимов + Профиль + Помощь
- Универсальный диспетчер `sq:*` callback'ов выбирает обработчик по FSM-состоянию (AI / hot-seat / online)

### Fixed
- Доска не обновлялась после некоторых ходов из-за идентичной разметки — теперь текст сообщения всегда меняется (счётчик хода), а `TelegramBadRequest` подавляется
- Все строки `BTN_BACK` и нижние клавиатуры корректно сбрасываются между режимами
